...
**Phase 5（上线准备与发布验收）** — 域名 ICP 备案、小程序备案、公安联网备案和隐私指引整改均已完成；生产登录/支付 fail-closed 与 TLS 代码整改已完成自动验证。2026-08-21 当前线上版本完成两笔 0.10 元真机支付，后台订单均已写入 `paidAt` 与真实 `wxTransactionId`，其中一笔已完成地址提交；这证明当前 JSAPI 支付与后端支付确认可用，但现有订单字段不能区分本次由有效回调还是 `pay-sync` 完成落库。微信订单发货管理真实启用、真实小程序码、其余安全整改、默认审核入口、正式业务参数、退款客服及分销合规仍是上线前事项。
...
- [x] 当前发布候选完成真实登录、下单、两笔 0.10 元真机支付与后端支付确认验收（2026-08-21；两笔均有 `paidAt`/真实 `wxTransactionId`）
- [x] 本地修复支付成功后的状态确认竞态：小程序仅在后端同时写入 `paidAt`/`wxTransactionId` 后进入地址页，未确认时禁止再次发起支付；服务端拒绝未确认支付订单填写地址，并允许迟到回调/对账补确认且保留 `address_pending`（2026-08-21；尚未部署、真机回归待完成）
...
- [x] 接入微信订单发货管理（2026-08-13 代码与自动验证完成：新增 `wx-trade` 模块，发货后 best-effort 上传 `upload_shipping_info`、失败落库不阻断、管理后台手动重试；运力编码表缓存；订单详情 path 与开通诊断接口；云托管 `WX_TRADE_MODE=real` 与微信侧开通待审批后启用，启用后需验证真实上传闭环）
...
- [x] 真机完成普通入口 → 登录 → 创建 1 本/0.10 元订单 → `wx.requestPayment` → 后端支付确认；两笔订单均落库真实微信支付单号，其中一笔完成地址/结果页（2026-08-21）
...
- [核心能力] 真实小程序码、微信退款及返点冲销尚未实现；微信订单发货管理代码已完成（2026-08-13），微信侧购物订单/发货管理开通状态与云端启用仍待单独验收，但已证实不是当前 JSAPI 支付的持续阻断项
- [生产安全] fail-closed、TLS、订单归属鉴权、日志脱敏和默认管理员整改代码已完成且已部署上线（2026-08-12）；线上管理员换密确认和真机人工验收尚未完成
- [支付状态一致性] 本地代码已修复原竞态：客户端保存“已提交微信支付、待后台确认”状态并重试 `pay-sync`，确认前不进入地址页且不再次下单支付；服务端拒绝 `pending_payment` 填地址，迟到回调/对账可补齐支付字段并保留 `address_pending`。server build、63/63 单元测试、44/44 全量 E2E、admin build、miniprogram build 已通过；部署与真机回归仍待执行
...
- [发布验收] 当前发布候选的两笔真实支付与后端确认已通过；有效回调与 `pay-sync` 来源分离、发货/售后/退款完整闭环尚未通过
...
- 2026-08-21：修复收货地址页输入文字在 Android 真机被上下裁切——根因是原生 `<input>` 只有垂直 `padding`、没有显式高度，内容区在部分真机上不足；统一设置输入框 `height/line-height: 88rpx`，垂直内边距改为 0，保留 20rpx 水平内边距。miniprogram build 与 `git diff --check` 通过；尚未部署，需在体验版真机确认占位文字和实际输入内容均完整显示
...
- 2026-08-13：微信订单发货管理（发货信息管理服务）代码完成——新增 `wx-trade` 模块：微信服务端 API 封装（Node https，鉴权优先读云托管免维护 token 文件、本地回退 `stable_token`），`WX_TRADE_MODE=real/mock/disabled`（缺省 disabled 跳过同步）。发货（`POST /api/shipments`）后 best-effort 调 `upload_shipping_info`，`shipments` 表新增 `companyId/wxSyncStatus/wxSyncError/wxSyncedAt` 字段（迁移 `AddShipmentWxSync1786636800000`），失败不阻断发货；管理后台发货弹窗改为微信运力编码下拉（`/api/wx-trade/express-companies` 缓存 1h），发货记录显示同步状态并支持手动重试（`POST /api/shipments/:id/wx-sync`）；另提供 `GET /api/wx-trade/status`（is_trade_managed 诊断）与 `POST /api/wx-trade/order-detail-path`（配置订单中心跳转 path，指向结果页并携带 `${商品订单号}`）。调研结论：「购物订单」分新旧两套（邮件申请的旧版需支付后上传购物详情，待审批确认后补；新版无需上传仅需配 path），发货信息管理服务独立强制、资金发货前冻结、默认 72h 发货时限。server build、56 个单元测试、43 个 E2E、admin build 全部通过；部署后保持 disabled，待购物订单审批开通后设 `WX_TRADE_MODE=real` 并验证真实上传闭环
