...
**Phase 3.2（真实化）：微信支付真实集成** — 云托管已部署支付模块与密钥；联调阻塞在 `POST /api/wx/login` 换 openid（已加固错误信息，待重新部署后定位 AppSecret/出网问题）
...
- [x] 真实密钥配置到云托管（用户侧已配 `WX_*` / `WX_PAY_ENABLED`）
- [x] 支付代码已推送到 GitHub `master`（`d18c2f9`），等待云托管重新部署生效
- [ ] 真机闭环：静默登录 openid → JSAPI 下单 → `wx.requestPayment` → 订单 `paid`
- [ ] 回调可达：`POST /api/wx/notify` 公网可达且验签落库；`pay-sync` 对账兜底
...
- Phase 3.2 联调：产品页/resolve 已通；点购买时 `ensureOpenid` → `POST /api/wx/login` 线上返回 500。已改为 https + 明确 503/400 错误信息。待部署后根据 toast/日志区分：AppSecret 错误 vs 云托管无法出网访问 `api.weixin.qq.com`。
...
- [x] 微信支付商户号与密钥：用户侧已配置到云托管（具体联调结果待真机验证）
