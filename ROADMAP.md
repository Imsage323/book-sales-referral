...
- [核心能力] 真实小程序码、微信退款及返点冲销尚未实现；微信订单发货管理代码已完成（2026-08-13），微信侧开通与云端启用待购物订单审批
...
- 2026-08-20：购物订单开通路径调研收官——「类目自助开通」路线验证失败：小程序已新增服务类目「商家自营 > 图书报刊/音像」并过审（原「文娱 > 出版物」为内容服务类目、与实物电商不匹配，保留不删），但购物订单仍提示"经营类目超出范围"，结论：对小类目商家购物订单实为**邮件特批制，无自助通道**，只能继续走 wechatpay_order@tencent.com 审批 + 商户平台工单/95017 催办。另：隐私指引因剪贴板用途说明不符被驳回，已按实际场景（地址页主动点击粘贴解析）给出改写文案待用户重新提交；wx-trade 模块代码（2026-08-13 完成）仍未提交部署，待用户确认后 commit+push
