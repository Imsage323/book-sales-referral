...
**Phase 5（上线准备与发布验收）** — 域名 ICP 备案和小程序备案已完成；生产登录/支付 fail-closed 与 TLS 代码整改已完成自动验证，尚未调整云托管环境变量或部署。线上仍运行 `/api/wx/diag` `wx-login-v3`，历史上已完成一次 0.01 元真机支付。以上不等于当前发布候选已验收：真实支付回调落库、微信订单发货管理、真实小程序码、隐私与合法域名、其余安全整改、默认审核入口、正式业务参数、退款客服及分销合规仍是上线前事项。
...
- [x] 线上 `/api/wx/diag` 返回 `wx-login-v3`，`hasAppid=true`、`hasSecret=true`
- [x] 本地 Phase 5 Task 2 代码完成：显式 `WX_LOGIN_MODE`/`WX_PAY_MODE`、生产启动校验、生产禁用 mock、TLS 校验恢复；`wx-login-v4` 尚未部署
...
Phase 5 Task 2 未修改真实 `src/server/.env`；2026-08-07 经用户确认补入本地直营销售方与默认商品参数。本地若需 mock，必须在 `NODE_ENV=development/test` 下显式设置 `WX_LOGIN_MODE=mock`、`WX_PAY_MODE=mock`；云托管部署前需经确认设置为 `real` 并复核全部支付变量。云托管同名 storefront 变量是否随新版服务生效，需部署后通过公开接口验证。

## 进行中

- [ ] Phase 5 上线整改与发布准备：Task 2、Task 3、Task 4 代码与自动验证完成，云托管、微信后台及真机人工验收待确认；其余任务待开发，详见 `docs/superpowers/plans/2026-08-06-phase5-miniprogram-launch-readiness.md`
...
- [x] 生产环境登录/支付配置 fail-closed，禁止占位 openid 和 mock 已支付（代码与自动测试完成；云托管配置和部署待确认）
...
- [x] 无 `scene` 默认商品入口、隐私授权、客服与审核体验（代码与自动验证完成；默认 storefront 环境变量、`request` 合法域名、客服能力、《小程序用户隐私保护指引》和真机验收待配置）
...
- [外部] 微信小程序备案尚未最终成功，备案完成前不能正式发布
- [业务决策] “一层直接推荐奖励”存在多级分销合规风险；结论明确前正式环境不得启用
- [业务参数] 正式售价、售后观察期、快递方案、客服与退款 SOP、入群二维码尚未确认
- [平台配置] `request` 合法域名、《小程序用户隐私保护指引》和微信订单发货管理尚未完成配置/接入
- [核心能力] 真实小程序码、微信退款及返点冲销尚未实现
- [生产安全] fail-closed、TLS、订单归属鉴权、日志脱敏和默认管理员整改代码已完成；云托管配置/部署、线上管理员换密确认和真机人工验收尚未完成
- [发布验收] 当前发布候选的有效支付回调落库与完整真机闭环尚未通过

## 待确认

- [ ] 正式售价（当前生产数据仍需核对，代码默认占位价为 0.01 元）
...
- [ ] 公安联网备案重新核查结果
- [x] 直营销售方 `DEFAULT_SELLER_CODE=SMFMSVV` 与默认在售商品 `DEFAULT_PRODUCT_ID=595f08d3-d92b-4c83-9445-1d743633ad1c` 已由用户确认并写入本地环境；云托管生效待新版部署验证
