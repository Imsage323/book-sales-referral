...
**本阶段完成，转入维护** — 用户于 2026-08-24 确认 8 月计划的销售与分销业务闭环（选书、下单、支付、地址、订单管理、分销记账、发货状态和端到端验收）已完成。支付恢复、微信发货管理真实接口、退款客服、正式售价及分销合规作为运维期补验，不作为本阶段未完成。

此前 Phase 5 记录：域名 ICP 备案、小程序备案、公安联网备案和隐私指引整改均已完成；用户于 2026-08-21 确认小程序已经上线，微信后台当前正式版本号与发布时间仍待只读复核。后端线上为配置版本 `server-038`；三笔 0.10 元真机支付及 `O-20260821-8634` 的普通入口、登录、1 本订单、真实支付、地址填写和结果页均已通过。真实小程序码归因与支付后奖励的两轮生产验收均已通过：第一轮订单 `O-20260822-1803` 生成 1 分销售奖励和 1 分上级直接推荐奖励；第二轮差异化规则生成 3 分销售奖励和 1 分上级直接推荐奖励，均为 `ESTIMATED`、无重复、不打款。管理后台返点规则/记录的空筛选缺陷已修复上线，并补充订单号展示。凭据轮换安全待办已记录，临时测试开关和测试专用规则暂不改动，等待用户后续处理。
...
- 8 月计划业务闭环已关闭。运维期补验：微信发货管理真实闭环、退款客服、正式售价、分销合规等，详见下方阶段计划未完成项。
...
- 2026-08-29：修复后台二维码列表看不到销售方、销售方列表看不到上级——根因：历史 schema 里 `seller_qrcodes`/`sellers` 同时存在 `sellerId`+`seller_id`、`parentId`+`parent_id` 两套列，代码写前者、`@JoinColumn` 关联后者（永远 NULL），关联查询恒为 null。修复：两个实体的 `@JoinColumn` 改为指向真实存值列；新增迁移 `DropRedundantSellerRelationColumns1788003200000`（按列名动态查外键并删除冗余列，容器启动自动执行，无需手工改库）；`InitialSchema` 同步收敛，新环境不再产生冗余列。历史数据无需处理（`sellerId` 列数据完整）。server build、80/80 单元测试、迁移在带旧表结构（双列+外键）的 3307 测试库真实执行成功、执行后 50/50 E2E 通过；已提交 `eaa0b99` 并推送，自动部署 `server-046` normal、流量 100%，线上实测：李引琴两条二维码正常显示销售方、李引琴上级正确显示企业直营；4 条更早的二维码指向已被硬删除的销售方（旧企业直营、测试下线），显示为空属预期
- 2026-08-29：修复管理后台订单管理显示「No Data」——根因：前端筛选框清空后会把 `status=`（空字符串）发给后端，`QueryOrderDto.status` 的 `@IsEnum` 校验不接受空串直接 400，页面将错误显示为无数据。修复：`query-order.dto.ts`、`query-seller.dto.ts`、`ledger-export.dto.ts` 的可选枚举字段统一加 `@Transform(({ value }) => value || undefined)`（与返点模块既有写法一致）。server build、80/80 单元测试、50/50 E2E 通过；已提交 `b3761b4` 并推送，自动部署 `server-044` normal、流量 100%，线上带空串参数实测返回 total=31，首行即 `O-20260829-9725`（¥0.10，`address_pending`）
