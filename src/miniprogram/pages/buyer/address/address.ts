...
Page({
  ...
  applyParse(text: string) {
    ...
    if (!next.recipient) missing.push('收件人');
    if (!next.phone) missing.push('手机');
    if (!next.province) missing.push('省');
    if (!next.city) missing.push('市');
    if (!next.district) missing.push('区');
    if (!next.addressDetail) missing.push('详细地址');
    // 详细地址只有「xx街道/镇/乡」时提醒补路名门牌
    const onlyAdminStreet = /^[\u4e00-\u9fa5]{1,12}(?:街道|镇|乡|苏木)$/.test(
      next.addressDetail || '',
    );
    if (missing.length) {
      wx.showToast({
  ...
  async onSubmit() {
    ...
    try {
      await patch('/buyer/orders/' + orderId + '/address', payload);
      wx.navigateTo({
        url:
          '/pages/buyer/result/result?orderId=' +
          orderId +
          '&groupQrcode=' +
